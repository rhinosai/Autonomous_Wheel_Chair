...
class VL53L8CXResultsData:
    def __init__(self, nb_target_per_zone: int) -> None:
        # Internal sensor silicon temperature */
        self.silicon_temp_degc: int = 0

        # Ambient noise in kcps/spads - originally # ifndef VL53L8CX_DISABLE_AMBIENT_PER_SPAD
        self.ambient_per_spad = [0] * VL53L8CX_RESOLUTION_8X8

        # Number of valid target detected for 1 zone - originally # ifndef VL53L8CX_DISABLE_NB_TARGET_DETECTED
        self.nb_target_detected = [0] * VL53L8CX_RESOLUTION_8X8

        # Number of spads enabled for this ranging - originally # ifndef VL53L8CX_DISABLE_NB_SPADS_ENABLED
        self.nb_spads_enabled = [0] * VL53L8CX_RESOLUTION_8X8

        # Signal returned to the sensor in kcps/spads - originally # ifndef VL53L8CX_DISABLE_SIGNAL_PER_SPAD
        self.signal_per_spad = [0] * (VL53L8CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Sigma of the current distance in mm - originally # ifndef VL53L8CX_DISABLE_RANGE_SIGMA_MM
        self.range_sigma_mm = [0] * (VL53L8CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Measured distance in mm - originally # ifndef VL53L8CX_DISABLE_DISTANCE_MM
        self.distance_mm = [0] * (VL53L8CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Estimated reflectance in percent - originally # ifndef VL53L8CX_DISABLE_REFLECTANCE_PERCENT
        self.reflectance = [0] * (VL53L8CX_RESOLUTION_8X8 * nb_target_per_zone)

        # Status indicating the measurement validity (5 & 9 means ranging OK) - originally # ifndef VL53L8CX_DISABLE_TARGET_STATUS
        self.target_status = [0] * (VL53L8CX_RESOLUTION_8X8 * nb_target_per_zone)
...
class VL53L8CX:
    def __init__(
        ...
        if self.VL53L8CX_MAX_RESULTS_SIZE < 1024:
            ...
            self.VL53L8CX_TEMPORARY_BUFFER_SIZE = self.VL53L8CX_MAX_RESULTS_SIZE

        if nb_target_per_zone == 1:
            self.VL53L8CX_START_BH = 0x0000000D
            self.VL53L8CX_METADATA_BH = 0x54B400C0
            self.VL53L8CX_COMMONDATA_BH = 0x54C00040
            self.VL53L8CX_AMBIENT_RATE_BH = 0x54D00104
            self.VL53L8CX_SPAD_COUNT_BH = 0x55D00404
            self.VL53L8CX_NB_TARGET_DETECTED_BH = 0xDB840401
            self.VL53L8CX_SIGNAL_RATE_BH = 0xDBC40404
            self.VL53L8CX_RANGE_SIGMA_MM_BH = 0xDEC40402
            self.VL53L8CX_DISTANCE_BH = 0xDF440402
            self.VL53L8CX_REFLECTANCE_BH = 0xE0440401
            self.VL53L8CX_TARGET_STATUS_BH = 0xE0840401
            self.VL53L8CX_MOTION_DETECT_BH = 0xD85808C0

            self.VL53L8CX_METADATA_IDX = 0x54B4
            self.VL53L8CX_SPAD_COUNT_IDX = 0x55D0
            self.VL53L8CX_AMBIENT_RATE_IDX = 0x54D0
            self.VL53L8CX_NB_TARGET_DETECTED_IDX = 0xDB84
            self.VL53L8CX_SIGNAL_RATE_IDX = 0xDBC4
            self.VL53L8CX_RANGE_SIGMA_MM_IDX = 0xDEC4
            self.VL53L8CX_DISTANCE_IDX = 0xDF44
            self.VL53L8CX_REFLECTANCE_EST_PC_IDX = 0xE044
            self.VL53L8CX_TARGET_STATUS_IDX = 0xE084
            self.VL53L8CX_MOTION_DETEC_IDX = 0xD858
            ...
            self.VL53L8CX_COMMONDATA_BH = 0x54C00040
            self.VL53L8CX_AMBIENT_RATE_BH = 0x54D00104
            self.VL53L8CX_NB_TARGET_DETECTED_BH = 0x57D00401
            self.VL53L8CX_SPAD_COUNT_BH = 0x55D00404
            self.VL53L8CX_SIGNAL_RATE_BH = 0x58900404
            self.VL53L8CX_RANGE_SIGMA_MM_BH = 0x64900402
            self.VL53L8CX_DISTANCE_BH = 0x66900402
            self.VL53L8CX_REFLECTANCE_BH = 0x6A900401
            self.VL53L8CX_TARGET_STATUS_BH = 0x6B900401
            self.VL53L8CX_MOTION_DETECT_BH = 0xCC5008C0

            self.VL53L8CX_METADATA_IDX = 0x54B4
            self.VL53L8CX_SPAD_COUNT_IDX = 0x55D0
            self.VL53L8CX_AMBIENT_RATE_IDX = 0x54D0
            self.VL53L8CX_NB_TARGET_DETECTED_IDX = 0x57D0
            self.VL53L8CX_SIGNAL_RATE_IDX = 0x5890
            self.VL53L8CX_RANGE_SIGMA_MM_IDX = 0x6490
            self.VL53L8CX_DISTANCE_IDX = 0x6690
            self.VL53L8CX_REFLECTANCE_EST_PC_IDX = 0x6A90
            self.VL53L8CX_TARGET_STATUS_IDX = 0x6B90
            self.VL53L8CX_MOTION_DETEC_IDX = 0xCC50
        ...
        self.offset_data = [0] * VL53L8CX_OFFSET_BUFFER_SIZE
        self.xtalk_data = [0] * VL53L8CX_XTALK_BUFFER_SIZE
        self.temp_buffer = [0] * self.VL53L8CX_TEMPORARY_BUFFER_SIZE
    ...
    def _poll_for_answer(
        ...
        while first or (self.temp_buffer[pos] & mask) != expected_value:
            ...
            if timeout >= 200:  # 2s timeout
                ...
                raise VL53L8CXException(VL53L8CX_MCU_ERROR)
            else:
                timeout += 1
    ...
    def _poll_for_mcu_boot(self) -> None:
        ...
        while timeout < 500:
            ...
            # print(f"Polling for mcu boot {hex(go2_status0)}")
            if go2_status0 & 0x80 != 0:
                go2_status1 = self.rd_byte(0x07)
                # print(f"Polling for mcu boot2 {hex(go2_status1)}")
                if go2_status1 & 0x01 != 0:
                    status |= VL53L8CX_STATUS_OK
                    break
        ...
        if status != 0:
            raise VL53L8CXException(status)
    ...
    def _send_offset_data(self, resolution: int) -> None:
        ...
        self.temp_buffer[:VL53L8CX_OFFSET_BUFFER_SIZE] = self.offset_data[:VL53L8CX_OFFSET_BUFFER_SIZE]

        # Data extrapolation is required for 4X4 offset
        if resolution == VL53L8CX_RESOLUTION_4X4:
            self.temp_buffer[16:16 + len(dss_4x4)] = dss_4x4[:]

            self.swap_buffer(self.temp_buffer, VL53L8CX_OFFSET_BUFFER_SIZE)
        ...
        for k in range(VL53L8CX_OFFSET_BUFFER_SIZE - 4):
            self.temp_buffer[k] = self.temp_buffer[k + 8]

        self.temp_buffer[0x1E0: 0x1E0 + 8] = footer[:]
        self.wr_multi(0x2e18, self.temp_buffer, VL53L8CX_OFFSET_BUFFER_SIZE)
        self._poll_for_answer(4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03)
    ...
    def _send_xtalk_data(self, resolution: int) -> None:
        ...
        self.temp_buffer[:VL53L8CX_XTALK_BUFFER_SIZE] = self.xtalk_data[0: VL53L8CX_XTALK_BUFFER_SIZE]
        ...
        if resolution == VL53L8CX_RESOLUTION_4X4:
            ...
            self.swap_buffer(self.temp_buffer, VL53L8CX_XTALK_BUFFER_SIZE)
        ...
        self.wr_multi(0x2cf8, self.temp_buffer, VL53L8CX_XTALK_BUFFER_SIZE)
        self._poll_for_answer(4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03)
    ...
    def init(self) -> None:
        ...
        # edited by jaeho
        self.wr_byte(0x7fff, 0x01)
        self.wr_byte(0x06, 0x01)
        self._poll_for_answer(1, 0, 0x21, 0xFF, 0x4)
        self.wr_byte(0x7fff, 0x00)
        ...
        self.wr_multi(0, self.buffers.VL53L8CX_FIRMWARE[0:0x8000], 0x8000)
        self.wr_byte(0x7fff, 0x0a)
        self.wr_multi(0, self.buffers.VL53L8CX_FIRMWARE[0x8000:0x10000], 0x8000)
        self.wr_byte(0x7fff, 0x0b)
        self.wr_multi(0, self.buffers.VL53L8CX_FIRMWARE[0x10000:0x15000], 0x5000)
        self.wr_byte(0x7fff, 0x01)
        ...
        #/* Firmware checksum */ added by jaeho
        self.rd_multi(0x812FFC & 0xFFFF, self.temp_buffer, 4)
        self.swap_buffer(self.temp_buffer,4)
        crc_checksum = int.from_bytes(self.temp_buffer[:4], byteorder='little')
    

        if crc_checksum != 0xc0b6c9e:
            print('crc_checksum :',crc_checksum)
            raise ValueError("Firmware checksum validation failed.")  

        # Get offset NVM data and store them into the offset buffer
        self.wr_multi(0x2fd8, self.buffers.VL53L8CX_GET_NVM_CMD, len(self.buffers.VL53L8CX_GET_NVM_CMD))
        self._poll_for_answer(4, 0, VL53L8CX_UI_CMD_STATUS, 0xff, 2)
        self.rd_multi(VL53L8CX_UI_CMD_START, self.temp_buffer, VL53L8CX_NVM_DATA_SIZE)
        self.offset_data[:VL53L8CX_OFFSET_BUFFER_SIZE] = self.temp_buffer[:VL53L8CX_OFFSET_BUFFER_SIZE]
        self._send_offset_data(VL53L8CX_RESOLUTION_4X4)

        # Set default Xtalk shape. Send Xtalk to sensor
        self.xtalk_data[:VL53L8CX_XTALK_BUFFER_SIZE] = self.buffers.VL53L8CX_DEFAULT_XTALK[:VL53L8CX_XTALK_BUFFER_SIZE]
        self._send_xtalk_data(VL53L8CX_RESOLUTION_4X4)

        # Send default configuration to VL53L8CX firmware
        self.wr_multi(0x2c34,
                      self.default_configuration,
                      len(self.buffers.VL53L8CX_DEFAULT_CONFIGURATION))
        self._poll_for_answer(4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03)

        self.dci_write_data(pipe_ctrl, VL53L8CX_DCI_PIPE_CONTROL, len(pipe_ctrl))

        if self.nb_target_per_zone != 1:
            tmp = [self.nb_target_per_zone]
            self.dci_replace_data(self.temp_buffer, VL53L8CX_DCI_FW_NB_TARGET, 16, tmp, 1, 0x0C)

        self.dci_write_data(single_range, VL53L8CX_DCI_SINGLE_RANGE, len(single_range))
    ...
    def get_power_mode(self) -> int:
        ...
        if tmp == 0x4:
            ...
            raise VL53L8CXException(VL53L8CX_STATUS_ERROR)
    ...
    def set_power_mode(self, power_mode: int) -> None:
        ...
        if power_mode != current_power_mode:
            if power_mode == VL53L8CX_POWER_MODE_WAKEUP:
                ...
                stored_mode = self.rd_byte(0x000F)
                if stored_mode  == 0x43:
                    self.wr_byte(0x000F, 0x40)
                self._poll_for_answer(1, 0, 0x06, 0x01, 1)
                if stored_mode == 0x43:
                    self.init()
            elif power_mode == VL53L8CX_POWER_MODE_SLEEP:
                self.wr_byte(0x7FFF, 0x00)
                self.wr_byte(0x09, 0x02)
                self._poll_for_answer(1, 0, 0x06, 0x01, 0)
            elif power_mode == VL53L8CX_POWER_MODE_DEEP_SLEEP:
                self.wr_byte(0x7FFF, 0x00)
                self.wr_byte(0x09, 0x02)
                self._poll_for_answer(1,0,0x06,0x01,0)
                self.wr_byte(0x000F,0x43)
    ...
    def start_ranging(self) -> None:
        ...
        output = [self.VL53L8CX_START_BH,
                  self.VL53L8CX_METADATA_BH,
                  self.VL53L8CX_COMMONDATA_BH,
                  self.VL53L8CX_AMBIENT_RATE_BH,
                  self.VL53L8CX_SPAD_COUNT_BH,
                  self.VL53L8CX_NB_TARGET_DETECTED_BH,
                  self.VL53L8CX_SIGNAL_RATE_BH,
                  self.VL53L8CX_RANGE_SIGMA_MM_BH,
                  self.VL53L8CX_DISTANCE_BH,
                  self.VL53L8CX_REFLECTANCE_BH,
                  self.VL53L8CX_TARGET_STATUS_BH,
                  self.VL53L8CX_MOTION_DETECT_BH]
        ...
        for i in range(total_output_len):
            ...
            if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
                print(f"vl53l8cx_start_ranging:  data_read_size={self.data_read_size}")

        self.data_read_size += 24
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"vl53l8cx_start_ranging:  final data_read_size={self.data_read_size}")

        output_bytes = long_array_to_bytes(output)
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"vl53l8cx_start_ranging: output_bytes={[hex(byte) for byte in output_bytes]}, {len(output_bytes)}")        
        self.dci_write_data(output_bytes, VL53L8CX_DCI_OUTPUT_LIST, len(output_bytes))

        header_config[0] = self.data_read_size
        header_config[1] = total_output_len + 1

        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"vl53l8cx_start_ranging:  header_config={header_config[0]:0{8}x} {header_config[1]:0{8}x}")

        header_config_bytes = long_array_to_bytes(header_config)
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:        
            print(f"vl53l8cx_start_ranging: header_config_bytes={[hex(byte) for byte in header_config_bytes]}, {len(header_config_bytes)}")
        self.dci_write_data(header_config_bytes, VL53L8CX_DCI_OUTPUT_CONFIG, len(header_config_bytes))

        output_bh_enable_bytes = long_array_to_bytes(output_bh_enable)
        if DEBUG_LOW_LEVEL_LOGIC_START_RANGING:
            print(f"vl53l8cx_start_ranging: output_bh_enable_bytes={[hex(byte) for byte in output_bh_enable_bytes]}, {len(output_bh_enable_bytes)}")
        self.dci_write_data(output_bh_enable_bytes, VL53L8CX_DCI_OUTPUT_ENABLES, len(output_bh_enable_bytes))
        ...
        self.wr_multi(VL53L8CX_UI_CMD_END - (4 - 1), cmd, len(cmd))
        self._poll_for_answer(4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03)
        ...
        if size != self.data_read_size:
            print(f"vl53l8cx_start_ranging: size={size} != data_read_size={self.data_read_size}, temp_buffer={[hex(byte) for byte in self.temp_buffer[:12]]}")
            raise VL53L8CXException(VL53L8CX_STATUS_ERROR)
        self.dci_read_data(self.temp_buffer,0xE0C4,8)
        if self.temp_buffer[0x6]!=0:
            print('Laser safety fault')
            raise VL53L8CXException(VL53L8CX_STATUS_LASER_SAFETY)
    ...
    def get_ranging_data(self) -> VL53L8CXResultsData:
        p_results = VL53L8CXResultsData(self.nb_target_per_zone)
        ...
        while i < self.data_read_size:
            ...
            elif not self.disable_ambient_per_spad and bh_ptr_idx == self.VL53L8CX_AMBIENT_RATE_IDX:
                to_ulong_array(p_results.ambient_per_spad, self.temp_buffer, i + 4, msize)
            elif not self.disable_nb_spads_enabled and bh_ptr_idx == self.VL53L8CX_SPAD_COUNT_IDX:
                to_ulong_array(p_results.nb_spads_enabled, self.temp_buffer, i + 4, msize)
            elif not self.disable_nb_target_detected and bh_ptr_idx == self.VL53L8CX_NB_TARGET_DETECTED_IDX:
                p_results.nb_target_detected[:msize] = self.temp_buffer[i + 4: i + 4 + msize]
            elif not self.disable_signal_per_spad and bh_ptr_idx == self.VL53L8CX_SIGNAL_RATE_IDX:
                to_ulong_array(p_results.signal_per_spad, self.temp_buffer, i + 4, msize)
            elif not self.disable_range_sigma_mm and bh_ptr_idx == self.VL53L8CX_RANGE_SIGMA_MM_IDX:
                to_uint_array(p_results.range_sigma_mm, self.temp_buffer, i + 4, msize)
            elif not self.disable_distance_mm and bh_ptr_idx == self.VL53L8CX_DISTANCE_IDX:
                to_uint_array(p_results.distance_mm, self.temp_buffer, i + 4, msize)
            elif not self.disable_reflectance_percent and bh_ptr_idx == self.VL53L8CX_REFLECTANCE_EST_PC_IDX:
        ...
        if not self.use_raw_format:
            ...
            if not self.disable_ambient_per_spad:
                for i in range(VL53L8CX_RESOLUTION_8X8):
    ...
    def get_resolution(self) -> int:
        self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_ZONE_CONFIG, 8)
        print(f"resolution:{self.temp_buffer[0x00] * self.temp_buffer[0x01]}")
        return self.temp_buffer[0x00] * self.temp_buffer[0x01]

    def set_resolution(self, resolution: int) -> None:
        if resolution == VL53L8CX_RESOLUTION_4X4:
            self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_DSS_CONFIG, 16)
            self.temp_buffer[0x04] = 64
            self.temp_buffer[0x06] = 64
            self.temp_buffer[0x09] = 4
            self.dci_write_data(self.temp_buffer, VL53L8CX_DCI_DSS_CONFIG, 16)
            ...
            self.dci_write_data(self.temp_buffer, VL53L8CX_DCI_ZONE_CONFIG, 8)
        elif resolution == VL53L8CX_RESOLUTION_8X8:
            self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_DSS_CONFIG, 16)
            ...
            raise VL53L8CXException(VL53L8CX_STATUS_INVALID_PARAM)
    ...
    def set_ranging_frequency_hz(self, frequency_hz: int) -> None:
        ...
        self.dci_replace_data(self.temp_buffer, VL53L8CX_DCI_FREQ_HZ, 4, frequency_hz_buf, 1, 0x01)

    def get_integration_time_ms(self) -> int:
        self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_INT_TIME, 20)
    ...
    def set_integration_time_ms(self, integration_time_ms: int) -> None:
        ...
        self.dci_replace_data(self.temp_buffer, VL53L8CX_DCI_INT_TIME, 20, buf, 4, 0x00)

    def get_sharpener_percent(self) -> float:
        self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_SHARPENER, 16)
    ...
    def set_sharpener_percent(self, sharpener_percent: int) -> None:
        ...
        self.dci_replace_data(self.temp_buffer, VL53L8CX_DCI_SHARPENER, 16, sharpener, 1, 0xD)

    def get_target_order(self) -> int:
        self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_TARGET_ORDER, 4)
        return self.temp_buffer[0x0]

    def set_target_order(self, target_order: int) -> None:
        if target_order == VL53L8CX_TARGET_ORDER_CLOSEST or target_order == VL53L8CX_TARGET_ORDER_STRONGEST:
            target_order_buf = [target_order]
            self.dci_replace_data(self.temp_buffer, VL53L8CX_DCI_TARGET_ORDER, 4, target_order_buf, 1, 0x0)
        else:
            raise VL53L8CXException(VL53L8CX_STATUS_INVALID_PARAM)

    def get_ranging_mode(self) -> int:
        self.dci_read_data(self.temp_buffer, VL53L8CX_DCI_RANGING_MODE, 8)

        if self.temp_buffer[0x01] == 0x1:
            return VL53L8CX_RANGING_MODE_CONTINUOUS

        return VL53L8CX_RANGING_MODE_AUTONOMOUS
    ...
    def set_ranging_mode(self, ranging_mode: int) -> None:
        ...
        if ranging_mode == VL53L8CX_RANGING_MODE_CONTINUOUS:
            ...
            single_range = 0x00
        elif ranging_mode == VL53L8CX_RANGING_MODE_AUTONOMOUS:
        ...
        self.dci_write_data(self.temp_buffer, VL53L8CX_DCI_RANGING_MODE, 8)

        buf = [single_range, 0, 0, 0]
        self.dci_write_data(buf, VL53L8CX_DCI_SINGLE_RANGE, 4)
    ...
    def dci_read_data(self,
        ...
        if (data_size + 12) > self.VL53L8CX_TEMPORARY_BUFFER_SIZE:
            raise VL53L8CXException(VL53L8CX_STATUS_ERROR)
        else:
            cmd[0] = (index >> 8) & 0xff
            cmd[1] = index & 0xff
            ...
            self.wr_multi(VL53L8CX_UI_CMD_END - 11, cmd, len(cmd))
            self._poll_for_answer(4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03)

            # Read new data sent (4 bytes header + data_size + 8 bytes footer)
            self.rd_multi(VL53L8CX_UI_CMD_START, self.temp_buffer, rd_size)
    ...
    def dci_write_data(self,
        ...
        address = VL53L8CX_UI_CMD_END - (data_size + 12) + 1
